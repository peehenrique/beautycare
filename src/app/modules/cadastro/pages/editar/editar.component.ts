...
export class EditarComponent implements OnInit {
  ...
  getCompanyByUserId(id) {
    const param = {uid: id};
    this.empresasService.loadEmpresaByUserId(param).subscribe(
      response => {
        console.log('Response loadEmpresaByUserId:', response);
        if (response.status) {
  ...
  submitCompanyInfo() {
    if (this.companyDataFormGroup.valid) {
      ...
      this.cadastroService.updateCompanyData(serializedForm).subscribe(
        response => {
    ...
    this.cadastroService.getCategorias().subscribe(
      response => {
        if (response.status) {
          ...
          this.getCompanyByUserId(this.globals.userId);
        }
      }
    );
  }
